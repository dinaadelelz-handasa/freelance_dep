...
{
  $ins=mysqli_query($con,"insert into ghiar_add_exist (ghiar_name,ghiar_serial,ghiar_number,ghiar_kmia,ghiar_moda,status) values('$_POST[ghiar_name]','$_POST[ghiar_serial]','$_POST[ghiar_number]',$_POST[ghiar_kmia],'$_POST[ghiar_moda]','$_POST[status]')");

  if($ins)
  {
    ?>
    <script>alert("تم اضافة القطعة للورشة بنجاح");
    window.location.replace("ghiar_add.php");
  </script>
    <?php
...
<h2 class="text-center">  قطع غيار المتوفرة في الورشة - اضافة جديد</h2>
...
<!-- <div class="row">
  <div class="col-25">
    ...
    <div class="row">
      ...
      <div class="col-75" style="direction:rtl">
        <label > جديدة</label>
        <input type="checkbox" name="status" value=" جديدة" checked="checked"> 
        <label >مستخدمة</label>
        <input type="checkbox" name="status" value="مستخدمة">
      </div>
    </div>
